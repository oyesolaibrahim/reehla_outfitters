...
console.log(location.pathname)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await axios.post('http://localhost:4000/api/jalab', formDataToSend, {
        headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const result = await axios.put(`http://localhost:4000/api/updatesinglejalab?jalabId=${jalabData._id}`, formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
