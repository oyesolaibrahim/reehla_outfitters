    ...
    <>
      <div className="md:flex justify-between xs:min-h-screen">
        <div className="flex flex-col space-y-3 justify-between">
          ...
          {myCarts.map(myCart => (
            <div key={myCart._id} className="bg-soft-ivory border-2 border-muted-gold rounded-lg max-h-62 min-h-62 sm:max-w-2/3 md:w-3/4 sm:min-w-36 md:min-w-36 p-10 m-5">
          ...
          {myCarts.length > 0 && (
            <button onClick={handleClearCart} className="sm:w-1/6 xs:w-1/2 ml-5 mb-3 bg-primary-brown text-soft-ivory py-3 px-5 rounded-lg">Clear Cart</button>
          )}
          {myCarts.length === 0 && (
          ...
          {myCarts.length > 0 && (
            <div className="xs:px-5 xs:pt-10 sm:pt-10 sm:px-5">
              <div className="bg-soft-ivory border-2 border-muted-gold p-5 lg:fixed md:w-1/3 lg:w-1/5 right-10 top-80 flex m-auto rounded-lg">
                <div>
                  <div className="mb-3">
                  ...
                  <Link to={"/checkout"} state={{ myCarts }}>
                    <p className="cursor-pointer font-bold rounded-lg bg-primary-brown text-soft-ivory py-3 px-5">CHECKOUT(₦{subTotal})</p>
                  </Link>
                </div>
              </div>
            </div>
