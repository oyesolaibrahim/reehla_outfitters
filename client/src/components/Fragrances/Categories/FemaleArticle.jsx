...
const Female_Fragrance = () => {
    ...
    useEffect(() => {
        const fetchFragrances = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_SERVER}/api/femalefragrance`);
        ...
        <>
            <div className="flex lg:pl-10 xs:justify-between md:pl-10 sm:pl-0 xs:px-2 flex-wrap sm:px-3 items-center">
                {objects.map(fragrance => (
                    <article key={fragrance._id} className={`bg-gray-800 border-2 text-gray-100 border-amber-600 xs:my-3 width vh ${adminToken ? "xsvhsp" : "xsvh"} xs:mx-1 xs:py-1 sm:p-10 lg:m-4 md:m-3 sm:my-5 sm:mx-2 xs:p-2`}>
                        ...
                        <div className="flex lg:flex-row items-center justify-between xs:flex-col space-y-3  xs:space-y-3">
...
export default Female_Fragrance;
